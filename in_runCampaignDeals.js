...
const { discoverCampaigns } = require("./in_discoverCampaigns");
...
function hasEnglishLanguage(productName = "") {
  const matches = productName.match(/\((.*?)\)/g);

  // No language tags at all → keep
...
async function scrapeCampaign(campaign) {
  ...
  for (const genre of GENRE_FACETS) {
    ...
    try {
      ...
      const beforeLanguageFilter = products.length;
      products = products.filter(p => hasEnglishLanguage(p.name));

      console.log(
        `    English filter: ${beforeLanguageFilter} -> ${products.length}`
      );
    } catch (err) {
